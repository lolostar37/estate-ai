...
export async function POST(req: Request) {
  try {
    ...
    const prices = filtered.map((item: any) => toNumber(item.dealAmount))
    ...
    filtered.forEach((item: any) => {
      ...
      const price = toNumber(item.dealAmount) / 10000
    ...
    const cycleAdjustedReturn = averageReturn * 0.6 + recentMomentum * 0.4
    ...
    const baseRate = Math.max(-0.01, cycleAdjustedReturn)
    ...
    const recentDeals = filtered
      ...
      .map((item: any) => toNumber(item.dealAmount) / 10000)
...
const blendedPrice =
  Math.round(
    ...
    const marketGapRate =
      recentMarketPrice > 0
        ? Math.round(((fairValue - recentMarketPrice) / recentMarketPrice) * 1000) /
          10
        : 0
...
현재 추정 시세: ${recentMarketPrice}억
AI 적정가 추정: ${fairValue}억
시세 대비 차이: ${marketGapRate}%

버블률 추정: ${bubble}%
투자 점수: ${investmentScore}점

연평균 성장률 추정: ${(annualGrowthRate * 100).toFixed(2)}%
변동성: ${(volatility * 100).toFixed(2)}%
하락 연도 비율: ${(negativeRatio * 100).toFixed(1)}%
예측 신뢰도: ${forecastConfidence}점
...
`,
  ...
  opinion,
      ...
      forecast: {
        ...
        negativeRatio: Math.round(negativeRatio * 1000) / 10,
        forecastConfidence,
      },
    })
  } catch (error) {
